...
import type { AdminProject, Locale } from '../models/adminModels';
...
function AdminProjectsPage() {
  ...
  useEffect(() => {
    ...
    getAdminProjects(controller.signal)
  ...
  const filteredItems = useMemo(() => items.filter((item) => {
    const text = `${item.title} ${item.shortDescription} ${item.slug}`.toLowerCase();
  ...
  async function togglePublish(item: AdminProject) {
    await (item.isPublished ? unpublishAdminProject(item.id) : publishAdminProject(item.id));
    await refresh();
  }

  async function deleteItem(item: AdminProject) {
  ...
  async function move(item: AdminProject, direction: -1 | 1) {
  ...
  return (
    <AdminShell>
      <div className="admin-page-header">
        ...
        <button className="admin-button" type="button" onClick={() => navigate('/admin/projects/new')}>
          New project
        </button>
      </div>
      ...
      {!loading && filteredItems.length === 0 && <p className="admin-muted">No projects match the current filters.</p>}
              ...
              <tr key={item.id}>
                ...
                <td>
                  <div className="admin-row-actions">
                    ...
                    <button type="button" onClick={() => navigate(`/admin/projects/${item.id}`)}>Edit</button>
