...
export class SayHiCooldown {
  ...
  async fetch(requestInfo: Request | string, init?: RequestInit): Promise<Response> {
    ...
    const body = (await request.json()) as { requestId: string; seconds: number };
    const now = Date.now();
    const cooldownUntil = (await this.state.storage.get<number>('cooldownUntil')) ?? 0;
